...
🔴 **先澄清一个常见误解：要的是 12 个 Google 账号，不是 12 台设备。**
一台手机可以切换账号，模拟器也可以。设备数量从来不是门槛。

**测试者实际要做的只有三件事**：点链接 → 接受邀请 → 装上别卸载。
**不需要每天使用**。所以话术可以很轻：「帮我点个链接装个 App，两周内别删，不用管它。」

**可行的凑人渠道**

| 渠道                       | 说明                                                                                                                                             |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 亲友 / 同事 / 同学         | 最主要来源，凑 8–10 个通常不难                                                                                                                   |
| 开发者互测社区             | `r/androiddev` 和一些 Discord 有 **closed testing exchange**，开发者互相加进对方轨道凑数。参与者是有真实历史的开发者账号，是被广泛使用的正常做法 |
| 自己已有的真实 Google 账号 | 工作 / 个人 / 备用账号，长期在用的没问题                                                                                                         |
| **Google Group**           | Play 的测试者列表支持直接填 Group 邮箱，人员进出不用回 Console 改，管理省事                                                                      |

**建议凑到 14–15 个账号**留冗余（人数掉到 12 以下会中断计时）。

### ⚠️ 不要批量注册小号凑数

技术上你确实可以开 5 个带 Google Play 的模拟器、注册 12 个新账号。但：

1. **生产访问申请是人工审核**，表单会问「测试中收到了什么反馈」「据此做了哪些改进」。12 个刚注册、同 IP、同设备指纹、零历史的账号 + 编造的反馈，很容易被判定为规避测试要求。
2. **代价不对称**：申请被拒还能再来，但若被认定为刻意规避政策，风险会落到**开发者账号本身**。
3. Google 近两年明确在收紧，被拒案例中这是主因之一。

### 💡 功能验证不要等封闭测试

| 轨道                 | 人数 | 生效   | 计入 12×14？    | 用途                             |
| -------------------- | ---- | ------ | --------------- | -------------------------------- |
| **Internal testing** | ≤100 | 立即   | ❌ **不计入**   | **你自己一个账号就能跑通全流程** |
| **Closed testing**   | 不限 | 需审核 | ✅ 计时的就是它 | 纯粹熬 14 天                     |

**正确顺序：先在内部测试轨道把功能验完、bug 修完，确认包是好的，再开封闭测试开始计时。**
否则 14 天熬到一半发现要改包，白等。
