...
| `npx expo export --platform web`               | ⏸ 已移除（web 能力 2026-07-24 删除） |
...
`tsc --noEmit` 0 错误 · `vitest run` 17/17 · `eslint .` 0/0 · `prettier --check` 全过（web 冒烟已于 2026-07-24 随 web 能力移除）。

### 待用户决策

1. 30 分钟汇报机制（会话内 / LaunchAgent / 飞书 bot）。
2. web 能力：已决策彻底移除（2026-07-24）。
