-- 0055: Persist the TaskKin account identity independently of auth-user and
-- household deletion. Apple appAccountToken must be a UUID, so the verified
-- Supabase auth UUID is the durable identity for the login-email account.
-- It is never a device or installation identifier.

alter table public.subscriptions
  add column if not exists owner_app_account_token uuid;

update public.subscriptions
  set owner_app_account_token = owner_user_id
  where owner_app_account_token is null
...
begin
  ...
  insert into public.subscriptions (
    ...
    environment, last_transaction_id, owner_user_id, owner_app_account_token
  ) values (
    p_household_id, p_original_transaction_id, p_plan, p_expires_at, 'active',
    p_environment, p_last_transaction_id, p_owner_user_id, p_owner_user_id
  )
  ...
  if v_sub.owner_app_account_token is not null and v_sub.owner_app_account_token <> p_owner_user_id then
    raise exception 'This Apple subscription is linked to another login-email account';
  end if;

  if v_sub.owner_user_id is null and v_sub.household_id is not null then
  ...
  update public.subscriptions
    set household_id = coalesce(household_id, p_household_id),
        ...
        owner_app_account_token = coalesce(owner_app_account_token, p_owner_user_id),
        updated_at = now()
    where id = v_sub.id;
